...
class Acquire():
    ...
    def _send_get(params, settings: Settings, cookie: Cookie):
        '''返回 json 格式数据'''
        # 优先尝试无代理连接
        print(f'[{Colors.CYAN}]尝试无代理连接获取作品数据...')
        try:
            ...
            print(f'[{Colors.YELLOW}]无代理连接失败，尝试使用代理连接...')
        except exceptions.ReadTimeout:
            print(f'[{Colors.YELLOW}]无代理连接超时，尝试使用代理连接...')
        
        # 如果无代理连接失败，尝试使用代理连接
        if hasattr(settings, 'proxy') and settings.proxy:
            print(f'[{Colors.CYAN}]尝试使用代理连接获取作品数据...')
            try:
                headers = HEADERS | {'Cookie': cookie._generate_str()}
                response = get(
                    POST_API,
    ...
    def _early_stop(self, earliest: date):
        ...
        if earliest and earliest > date.fromtimestamp(self.cursor / 1000):
            self.finished = True

    def request_items(self, sec_user_id: str, earliest: date, settings: Settings, cookie: Cookie):
        '''获取账号作品数据并返回'''
        items = []
        # 禁用Progress对象，避免与Streamlit显示冲突
        self.cursor = 0
        self.finished = False
