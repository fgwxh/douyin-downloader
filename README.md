...
本项目基于 [https://github.com/GuanDD123/douyin_download](https://github.com/GuanDD123/douyin_download) 进行扩展开发而来，在原有功能基础上增加了Web界面、作品管理、视频预览等功能。
...
Windows用户可以使用 `start_ui.bat` 脚本快速启动应用：
...
- **自动启动服务器**：在新窗口中启动uvicorn服务器
- **自动打开浏览器**：等待服务器启动后，自动打开浏览器访问前端页面
- **一键停止**：按任意键停止服务器并退出
- **简单易用**：无需手动输入命令，适合快速使用

#### 5.3 注意事项

- **仅支持Windows系统**：此脚本为批处理文件，仅在Windows系统上运行
- **需要Python环境**：确保已安装Python 3.8+和所有依赖
- **端口占用**：确保8501端口未被占用
- **权限要求**：需要有足够的文件读写权限
